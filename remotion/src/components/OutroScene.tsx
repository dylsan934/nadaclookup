...
export const OutroScene: React.FC<{ tagline: string }> = ({ tagline }) => {
  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: 48, fontSize: 64, fontWeight: 800, color: theme.white, letterSpacing: -1.5, opacity: urlIn, transform: `translateY(${interpolate(urlIn, [0, 1], [16, 0])}px)` }}>
        nadaclookup.com
      </div>
      <div style={{ marginTop: 24, fontSize: 30, color: "rgba(255,255,255,0.75)", fontWeight: 500, opacity: tagIn, transform: `translateY(${interpolate(tagIn, [0, 1], [16, 0])}px)` }}>
        {tagline}
      </div>
    </AbsoluteFill>
  );
